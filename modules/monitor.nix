...
let
  ...
  ifElseEmpty = t: v: if t then v else "";
...
in
{
  ...
  config =
    let
      edids = filter (m: m.edid != null) (attrValues cfg);
    in
    mkMerge [
      (mkIf ((length edids) != 0) {
        hardware.firmware = map (
          s:
          with s;
          (pkgs.runCommand "${name}-edid-override" { compressFirmware = false; } ''
            mkdir -p "$out/lib/firmware/edid"
            cp "${edid}" "$out/lib/firmware/edid/${name}.bin"
          '')
        ) edids;

        boot.kernelParams = map (s: with s; "drm.edid_firmware=${name}:edid/${name}.bin") edids;
      })
      {
        services.xserver.xrandrHeads = map (
          m: with m; {
            output = if (xorgName != "") then xorgName else name;

            inherit primary;
            monitorConfig = ''
              ${xorgModeline}
              ${
                if (xorgModeline != "") then
                  let
      ...
      (mkIf (elem "niri" config.meow.workstation.environment) {
        meow.workstation.extraNiriConfig = map (
          mon:
          let
            inherit (mon)
              name
                ...
                concatStringsSep "\n" (
                  map (
                    ...
                    let
                      ...
                      emacsBool = bool: if bool then "t" else "nil";
                    in
                    ''
