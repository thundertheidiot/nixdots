...
(use-package consult
  ...
  (:states '(normal visual motion) :keymaps 'override :prefix "SPC"
    "sg" '((lambda () (interactive)
	     (when (consult-ripgrep (expand-file-name ""))
		 ...
		 (setq evil-ex-search-direction 'forward)))) :wk "grep")
...
(transient-define-prefix empv-menu ()
  ...
  [["Playback"
    ("p" "toggle" empv-toggle)
    ("v" "toggle video" empv-toggle-video)

    ("j" "previous" empv-playlist-prev)
    ("k" "next" empv-playlist-next)

    ("Y" "search youtube" empv-youtube)
    ("f" "play file" empv-play-file)
    ("s" "select from playlist" empv-playlist-select)

    ("x" "close mpv" empv-exit)
    ]
   ["Config"
    ("y" "toggle single" empv-toggle-file-loop)
    ("r" "toggle repeat" empv-toggle-playlist-loop)
    
    ("-" "volume down" empv-volume-down)
    ("=" "volume up" empv-volume-up)
    ]
   ])
...
(transient-define-prefix mpd-menu ()
  "Transient menu for empv."
  [["Playback"
    ("p" "toggle" simple-mpc-toggle)
    
    ("j" "previous" simple-mpc-prev)
    ("k" "next" simple-mpc-next)
    ]
   ["Config"
    ("y" "toggle single" (lambda () (interactive)
			   (simple-mpc-call nil "single")))
    ("r" "toggle repeat" simple-mpc-toggle-repeat)

    ("-" "volume down" simple-mpc-decrease-volume)
    ("=" "volume up" simple-mpc-increase-volume)
    ]
   ])

(defun media-menu--toggle-mpv-or-mpd ()
  (interactive)
  (empv--send-command 
...
(transient-define-prefix media-menu ()
  ...
  [["MPV"
   ...
   ["Playback"
    ("p" "toggle playback" media-menu--toggle-mpv-or-mpd)
    ]]
   )
#+end_src
