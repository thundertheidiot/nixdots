...
(use-package corfu
  ...
  (:states '(normal visual insert) :keymaps 'corfu-mode-map
	   "C-j" nil
	   "C-k" nil)
  (:states '(normal visual insert) :keymaps 'corfu-map
	   "RET" nil
	   "<up>" nil
	   "<down>" nil
	   "C-j" #'corfu-next
...
(defun eshell/nix-shell (&rest args)
  "Calling nix-shell without explicitly running a shell breaks when entering a new terminal.
This alias fixes the issue."
  (let ((args (if (or (not args) (member "--run" args))
		  args
		(append args '("--run" "fish")))))
    (apply #'eshell-exec-visual "nix-shell" args)))
...
(use-package ellama
  ...
  (setopt ellama-providers '(
			     ("llama3.2" . (make-llm-ollama
					  ...
					  :embedding-model "nomic-embed-text"))
			     ("qwen2.5-coder:14b" . (make-llm-ollama
			     ...
			     ("qwen2.5-coder:3b" . (make-llm-ollama
					  ...
					  :embedding-model "nomic-embed-text"))
			     ("exaone3.5:2.4b" . (make-llm-ollama
					    :chat-model "exaone3.5:2.4b"
					    :embedding-model "nomic-embed-text"))
			     ("exaone3.5:7.8b" . (make-llm-ollama
					    :chat-model "exaone3.5:7.8b"
					    :embedding-model "nomic-embed-text"))
			     ("exaone3.5:32b" . (make-llm-ollama
					    :chat-model "exaone3.5:32b"
					    :embedding-model "nomic-embed-text")))))
...
(use-package dired
  ...
  (:keymaps 'dired-mode-map :states '(normal insert visual motion)
    ...
    "C-m" (lambda () (interactive) (empv-play (dired-get-filename))))
...
button event."
	...
	(let* ((monitor-attrs (frame-monitor-attributes))
	       (geometry (cdr (assoc 'workarea monitor-attrs)))
	       (mon-x (nth 0 geometry))
	       (mon-y (nth 1 geometry)))
