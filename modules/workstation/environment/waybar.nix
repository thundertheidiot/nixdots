...
{
  options = {
    meow.workstation.waybarDiskFilter = mkOpt (listOf str) [ ] {
  ...
  config = mkIf (work && (elem "niri" env)) (homeModule {
    programs.waybar.settings =
      let
        diskName = name: if name == null then "NULL" else replaceStrings [ "/" ] [ "_" ] name;
        ...
        fileSystems = filterAttrs (_: d: !elem d.mountPoint filterList) config.fileSystems;
      ...
      in
      {
        main = {
          ...
          modules-right =
            unique (mapAttrsToList (_: fs: "disk#${replaceStrings [ "/" ] [ "_" ] fs.device}") fileSystems)
