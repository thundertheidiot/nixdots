...
in
{
  ...
  config = mkIf (config.meow.program.discord && cfg.enable) (
    let
      defaultSettings = {
      ...
      finalUserSettings =
        if cfg.override.userSettings then
          mergeAttrsList [
            cfg.userSettings
    ...
    in
    {
      meow.home.configFile."vesktop/quickCss.css" = {
        enable = true;
        text = if cfg.override.quickCss then cfg.quickCss else defaultQuickCss + cfg.quickCss;
      };
